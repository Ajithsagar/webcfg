...
void test_createHeader_doc_header_max_data() {
	...
	if (!subdoc_found)
		printf("  'Doc-Name' header NOT found!\n");
	else
		printf("  'Doc-Name' header found!\n");
...
void test_createHeader_primary_sync() {
	...
	struct curl_slist *temp = headers_list;
	printf("Checking headers in headers_list for primary sync:\n");
	...
	// Cleanup
	if (transID) {
		printf("Freeing transID\n");
		free(transID);
		transID = NULL;
	}
	
	if (headers_list) {
		printf("Freeing headers_list\n");
		curl_slist_free_all(headers_list);
		headers_list = NULL;
	}
...
void test_createHeader_supplementary_sync() {
	...
	struct curl_slist *temp = headers_list;
	printf("Checking headers in headers_list for supplementary sync:\n");
